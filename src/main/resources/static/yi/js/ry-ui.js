        ...
        operate: {
            ...
            show: function(id) {
                ...
                if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.showUrl.replace("{id}", id);
                    $.modal.open("修改" + table.options.modalName, url);
                } else {
                    $.modal.open("修改" + table.options.modalName,table.options.showUrl.replace("{id}", id));
                }
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.SkuMan_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
